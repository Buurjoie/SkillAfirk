...
const [searchQuery, setSearchQuery] = useState("");
 ...
 console.log(id);
  ...
  const title = `Formations en ${id.replace(/-/g, " ")} - SkillAfrik`;
  const description = `Explore nos formations en ${id.replace(/-/g, " ")}, conçues pour les jeunes africains en reconversion, autodidactes ou étudiants.`;
  ...
  const fetcheData = async () => {
   ...
   .then((data) => {
    const domaine = data.domaines.find((domaine) => domaine.title.toLowerCase().replace(/\s+/g, "-") === id );
    console.log("Données domaine:", domaine);
     setDomaines(domaine);
   })
   .catch((error) => {
          ...
          <div className="max-w-3xl mx-auto flex flex-col md:flex-row gap-4">
            <div className="flex-1 relative">
              <input
                ...
                className="w-full px-6 py-4 rounded-lg text-gray-800 border-none shadow-lg pl-12"
                value={searchQuery}
